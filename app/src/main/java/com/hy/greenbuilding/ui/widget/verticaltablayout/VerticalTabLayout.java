package com.hy.greenbuilding.ui.widget.verticaltablayout;
...
import com.hy.greenbuilding.ui.widget.verticaltablayout.adapter.TabAdapter;
import com.hy.greenbuilding.ui.widget.verticaltablayout.util.DisplayUtil;
import com.hy.greenbuilding.ui.widget.verticaltablayout.util.TabFragmentManager;
import com.hy.greenbuilding.ui.widget.verticaltablayout.widget.QTabView;
...
public class VerticalTabLayout extends ScrollView {
    ...
    public void addTab(TabView tabView) {
        if (tabView != null) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                tabView.setForeground(null);
            }
            // ⭐ 另一次尝试：确保 Tab 的 Background 被完全控制，不自动添加点击状态
            tabView.setClickable(true); // 保持可点击性，但控制背景
    ...
    public void setTabSelected(final int position) {
        ...
        if (HyApplication.isLocking) {
            ToastUtil.showToast(mContext, mContext.getResources().getString(R.string.device_locked));
            return;
        }
...
//    }
    ...
    public void setTabAdapter(MyTabAdapter adapter, boolean param) {
        removeAllTabs();
        if (adapter != null) {
            mTabAdapter = adapter;
            for (int i = 0; i < adapter.getCount(); i++) {
                addTab(new QTabView(mContext).setIcon(adapter.getIcon(i))
                        .setTitle(adapter.getTitle(i)).setBadge(adapter.getBadge(i))
                        .setBackground(adapter.getBackground(i)),param);
            }
        }
    }

    private void addTab(QTabView tabView, boolean param) {
        if (tabView != null) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                tabView.setForeground(null);
            }
