...
import com.hy.greenbuilding.protocol.IDoorStatusInfo;
...
public class OutDoorStatusInfo implements IDoorStatusInfo {
    ...
    public OutDoorStatusInfo(byte[] data) {
        ...
        Log.i("info", "outdoor data--" + Hex.bytesToHexString(data));
    }
    ...
    @Override
    public BigDecimal inDoorTemp1() {
        byte[] b1 = Arrays.copyOfRange(statusData, 8, 10);
        short temp = ByteUtils.byteArrayToShort(b1);
        BigDecimal bigDecimal1 = new BigDecimal((float) temp / 10).setScale(1, BigDecimal.ROUND_DOWN);
        BigDecimal bigDecimal2 = bigDecimal1.setScale(0, BigDecimal.ROUND_DOWN);
        hfTopic.setInHotTemp(bigDecimal2.shortValue());
        return bigDecimal1;
    }
    ...
    public BigDecimal moduleTemp() {
        byte[] b1 = Arrays.copyOfRange(statusData, 20, 22);
        short temp = ByteUtils.byteArrayToShort(b1);
    ...
    public BigDecimal outFunSpeed() {
        int temp = ByteUtils.byteArrayToInt(Arrays.copyOfRange(statusData, 22, 24),
                0, Arrays.copyOfRange(statusData, 22, 24).length);
    ...
    public byte[] faultMessage2() {
        byte[] bytes = Arrays.copyOfRange(statusData, 30, 32);
        hxTopic.setErrorCode0(bytes);
        return bytes;
    }

    @Override
    public byte[] faultMessage3() {
        //er
        byte[] bytes = Arrays.copyOfRange(statusData, 32, 34);
        hxTopic.setErrorCode(bytes);
        return bytes;
    }
    ...
    public int defrostStatus() {
        byte[] bytes = Arrays.copyOfRange(statusData, 36, 37);
        int status = ByteUtils.byteArrayToInt(bytes,
                0, bytes.length);
        return status;
    }

    //000000000000FED4FED4000000000000000000000000000000000000000000000000000001000900000000
    @Override
    public String functionTestValue() {
        byte[] bytes = Arrays.copyOfRange(statusData, 37, 39);
    ...
    public int frequencyTestValue() {
        ...
        int value = ByteUtils.byteArrayToInt(bytes,
                0, bytes.length);
        return value;
    }
...
//    0000  0000  00A0  0109  0102  00E6  FFB0  01CC  01F4  00E6  014A  0003 003C 0078 0001 0001 /0000 0003 00 0000 0000 0000
}
